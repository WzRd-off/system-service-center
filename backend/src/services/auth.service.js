...
class AuthService {
  async register({ email, phone, password, role, companyName, edrpou, contact_person, address, firstName, lastName }) {
    ...
    if (role === ROLE.BUSINESS && !companyName?.trim()) {
      throw ApiError.badRequest('Company name is required for business clients');
    }
    if (role === ROLE.BUSINESS && !edrpou?.trim()) {
      throw ApiError.badRequest('EDRPOU is required for business clients');
    }
    if (role === ROLE.BUSINESS && !contact_person?.trim()) {
      throw ApiError.badRequest('Contact person is required for business clients');
    }
  ...
  async _createProfile(create_user, user, { companyName, edrpou, contact_person, address, firstName, lastName }) {
    switch (user.role) {
      case ROLE.CLIENT:
        await create_user.query(
          ...
          [user.id, firstName || null, lastName || null, address || null]
        );
        return;
      case ROLE.TECHNICIAN:
        await create_user.query(
          `INSERT INTO technician_profiles (user_id, first_name, last_name)
           VALUES ($1, $2, $3)
           ON CONFLICT (user_id) DO NOTHING`,
      ...
      case ROLE.BUSINESS:
        await create_user.query(
          `INSERT INTO business_client_profiles (user_id, company_name, edrpou, contact_person, address)
           VALUES ($1, $2, $3, $4, $5)
           ON CONFLICT (user_id) DO NOTHING`,
          [user.id, companyName, edrpou, contact_person, address || null]
        );
        return;
      default:
        return;
  ...
  async login({ login, email, password }) {
    ...
    if (!identifier) throw ApiError.unauthorized('Invalid login or password');
    ...
    if (identifier.includes('@')) {
      ...
      const compact = this._normalizePhone(identifier);
      ({ rows } = await db.query(
        `SELECT * FROM users
         WHERE phone = $1
            OR REPLACE(REPLACE(phone, ' ', ''), '+', '') = $2`,
        [identifier, compact]
      ));
    }
    ...
    if (!user || !(await bcrypt.compare(password, user.password))) {
      throw ApiError.unauthorized('Невірний логін або пароль');
    }
  ...
  async changePassword({ userId, currentPassword, newPassword }) {
    ...
    if (!ok) throw ApiError.badRequest('Invalid current password');
